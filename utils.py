...
class Face:
    ...
    def crop(self, size, factor=1.0, align_mode="arcface"):
        ...
        Args:
            ...
            align_mode: 对齐模式 ("arcface", "centered")
        ...
        # 根据对齐模式选择不同的处理方法
        if align_mode == "centered":
            print(f"使用centered模式裁剪人脸，使用factor={factor}")
            
            # 计算人脸中心点
            if self.kps is not None:
                # 如果有关键点，使用关键点的中心
                face_center = np.mean(self.kps, axis=0)
            elif self.landmarks is not None:
                # 如果有landmarks，使用landmarks的中心
                face_center = np.mean(self.landmarks, axis=0)
            else:
                # 否则使用边界框的中心
                face_center = np.array([(self.x1 + self.x2) / 2, (self.y1 + self.y2) / 2])
            
            # 计算人脸尺寸
            if self.kps is not None:
                # 使用关键点计算人脸尺寸
                x_min, y_min = np.min(self.kps, axis=0)
                x_max, y_max = np.max(self.kps, axis=0)
                face_width = x_max - x_min
                face_height = y_max - y_min
                face_size = max(face_width, face_height)
            elif self.landmarks is not None:
                # 使用landmarks计算人脸尺寸
                x_min, y_min = np.min(self.landmarks, axis=0)
                x_max, y_max = np.max(self.landmarks, axis=0)
                face_width = x_max - x_min
                face_height = y_max - y_min
                face_size = max(face_width, face_height)
            else:
            ...
            print(f"使用centered模式完成裁剪，人脸中心位于画布中心，factor={factor}")
            return M, crop
        elif align_mode == "arcface":
            ...
            # 计算人脸中心点
            face_center = np.mean(src_pts, axis=0)
            
            # 根据factor调整裁剪范围
            # 使用factor，值越大，缩放越小（人脸占比越大）
            S = np.array([
                [factor, 0, 0],
                [0, factor, 0],
                [0, 0, 1]
